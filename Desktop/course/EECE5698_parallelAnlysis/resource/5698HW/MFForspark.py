...
def joinAndPredictAll(R,U,V,N):
    ...
    """
    predictAll  = R.map(lambda (i,j,rij): (i,(j,rij))).join(U,numPartitions = N)\
...
def SE(joinedRDD):
    ...
    """
    SE = joinedRDD.map(lambda (i,j,delta,ui,vj):delta**2)\
...
def normSqRDD(profileRDD,param):
    """ Receives as input an RDD of profiles (e.g., U) as well as a parameter (e.g., λ) and computes the square of norms:
        ...
        λ Σ_i ||ui||_2^2         
    """
    normSqRDD = profileRDD.map(lambda (i,u): np.dot(u,u))\
		.reduce(lambda x,y:x+y)
    return param*normSqRDD
...
def adaptU(joinedRDD,gamma,lam,N):
    ...
    """
    Ui = joinedRDD.map(lambda (i,j,delta,ui,vj): (i,(ui,gradient_u(delta,ui,vj))))\
		.reduceByKey(lambda x,y:(x[0],x[1]+y[1]),numPartitions=N)
    UiNew = Ui.mapValues(lambda (ui,grad): ui-gamma*(grad+2*lam*ui))
    return UiNew
...
def adaptV(joinedRDD,gamma,mu,N):
    ...
    """
    Vj = joinedRDD.map(lambda (i,j,delta,ui,vj): (j,(vj,gradient_v(delta,ui,vj))))\
                .reduceByKey(lambda x,y:(x[0],x[1]+y[1]),numPartitions=N)
    VjNew = Vj.mapValues(lambda (vj,grad): vj-gamma*(grad+2*mu*vj))
    return VjNew
...
if __name__=="__main__":
    ...
    for mu in range(1,51):
        ...
        cvSE.append(np.mean(cross_val_rmses))
    print cvSE
    fi = open('outfull','wb')
    for item in cvSE:
        fi.write('%.5f\n' % item)
