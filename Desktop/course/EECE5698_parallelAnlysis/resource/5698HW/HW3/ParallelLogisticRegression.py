...
def test(dataRDD,beta):
    ...
    NegetiveS = totalscore.filter(lambda (x,y) : x <= 0 )
    P = PositiveS.count()
    N = NegetiveS.count()
    ...
    FP = PositiveS.filter(lambda (x,y) : float(y) == -1.0)\
                  .count()
    TN = NegetiveS.filter(lambda (x,y) : float(y) == -1.0)\
                  .count()
    FN = NegetiveS.filter(lambda (x,y) : float(y) == 1.0)\
                  .count()
...
def train(dataRDD,beta_0,lam,max_iter,eps,f,test_data=None):
    ...
    while k<max_iter and gradNorm > eps:
        ...
        if test_data == None:
            print>>f, 'k = ',k,'\tt = ',time()-start,'\tL(β_k) = ',obj,'\t||∇L(β_k)||_2 = ',gradNorm,'\tgamma = ',gamma
        else:
            acc,pre,rec = test(test_data,beta)
            print>>f, 'k = ',k,'\tt = ',time()-start,'\tL(β_k) = ',obj,'\t||∇L(β_k)||_2 = ',gradNorm,'\tgamma = ',gamma,'\tACC = ',acc,'\tPRE = ',pre,'\tREC = ',rec
        k = k + 1
...
if __name__ == "__main__":
    ...
    beta0 = SparseVector({})
    print>>f, 'Training on data from',args.traindata,'with λ =',args.lam,', ε =',args.eps,', max iter = ',args.max_iter
    beta, gradNorm, k = train(traindata,beta_0=beta0,lam=args.lam,max_iter=args.max_iter,eps=args.eps,test_data=testdata,f=f) 
    print>>f, 'Algorithm ran for',k,'iterations. Converged:',gradNorm<args.eps
    print>>f, 'Saving trained β in',args.beta
